...
## ⚠️ CRITICAL: Generator/Scanner Compatibility
...
- **No fallback**: If scanner can't decode QR codes → mission failure
...
# Test matrix:
# - Compression: brotli level 11 (ONLY algorithm supported)
...
**If ANY test fails → DO NOT DEPLOY. System is broken.**
...
- [ ] **Brotli-11 decompression works** - ONLY compression algorithm, must succeed
...
- [ ] **Metadata extraction works** - Filename, size, compression=brotli, encryption=aes256gcm
- [ ] **Air-gap verified** - ZERO network communication during transfer
- [ ] **Native browser API verified** - Brotli decompression uses built-in DecompressionStream
...
❌ **Scanner removes Brotli decompression support**
- Example: Scanner breaks native Brotli API, but generator ONLY uses Brotli-11

❌ **Generator changes chunk size WITHOUT updating scanner**
- Example: Generator uses 3000-byte chunks, scanner expects max 2272

❌ **Scanner changes completion detection logic**
- Example: Scanner requires sequential scanning, but generator assumes any order

❌ **Generator changes metadata fields WITHOUT updating scanner**
...
2. If binary format changes → update scanner parser in lockstep
3. If compression implementation changes → verify scanner decompression still works
4. If metadata fields change → update scanner metadata parser
5. Run full compatibility test suite (Brotli-11, all file types and sizes)
...
3. Verify Brotli-11 decompression still supported (native browser API)
...
1. If Protocol V4 needed → create NEW generator AND scanner
...
**Cause**: Browser doesn't support native Brotli decompression (unlikely - all modern browsers support it)
**Fix**: Verify browser version supports DecompressionStream API, test decompression manually in console
...
- ✅ **Maximum compression** (Brotli level 11 ONLY - BEST compression ratio, native browser support)
- ✅ **Optimal chunk size** (2272 bytes for QR-40M with binary format)
- ✅ **Error correction balance** (Medium for best capacity/reliability)
- ✅ **Single algorithm** (Brotli-11 only - best compression + simplicity for air-gap reliability)
...
| `compression` | string | ✅ | `brotli` (ONLY - level 11 always) |
| `compression_level` | number | ✅ | Always 11 (maximum compression) |
...
- `brotli`: **Brotli level 11** (ONLY compression algorithm supported)
  - Compression ratio: 20-30% (BEST available - superior to Zstd)
  - Speed: Slow compression (~5-10 MB/s), Fast decompression (~50-100 MB/s)
  - Result: Minimum number of QR codes (5-10% fewer than Zstd)
  - **Why ONLY Brotli-11?**
    - ✅ **Best compression ratio** = fewest QR codes (5-10% better than Zstd)
    - ✅ **Native browser support** = zero external libraries for scanner (simpler air-gap)
    - ✅ **Simpler deployment** = no WASM bundles, built into all modern browsers
    - ✅ **Single code path** = easier to verify compatibility
    - ✅ **Fewer failure points** = no external dependencies in military deployment
    - ⚠️ **Slower compression** = acceptable (compress once, scan many times)
...
```python
# Example: 10MB file with Brotli-11 compression

original_size = 10 * 1024 * 1024  # 10MB = 10,485,760 bytes
compression_ratio = 0.25  # Brotli-11 achieves ~25% of original (better than Zstd!)
compressed_size = original_size * compression_ratio  # 2,621,440 bytes
...
data_qrs = ceil(data_to_split / chunk_size)  # 1150 QRs

total_qrs = header_qrs + data_qrs  # 1151 QRs

# Compare to NAIVE approach (JSON + base64 everywhere):
# chunk_size_naive = 1000 bytes (base64 overhead)
# naive_qrs = ceil(compressed_size / 1000)  # 2622 QRs
#
# SAVINGS: 56% fewer QR codes! (1151 vs 2622)
#
# Compare to Zstd-22 (30% ratio = 3.0MB):
# zstd_qrs = 1381 QRs
# BROTLI ADVANTAGE: 230 fewer QR codes! (1151 vs 1381) = 115 seconds faster at 2 FPS!
```

#### Compression Impact on QR Count

| File Type | Size | No Compression | Brotli-11 | Zstd-22 (old) | Brotli Advantage |
|-----------|------|----------------|-----------|---------------|------------------|
| Text (.txt) | 1MB | 463 QRs | **70 QRs** | 81 QRs | 14% fewer! |
| Code (.py) | 500KB | 232 QRs | **36 QRs** | 41 QRs | 12% fewer! |
| JSON (.json) | 2MB | 926 QRs | **116 QRs** | 130 QRs | 11% fewer! |
| Binary (.bin) | 1MB | 463 QRs | **397 QRs** | 405 QRs | 2% fewer |
| Image (.jpg) | 1MB | 463 QRs | **463 QRs** | 463 QRs | 0% (skip compression!) |

**Key Insight**: Brotli-11 provides the BEST compression for QR codes! Text/code/JSON files benefit enormously, with 10-15% fewer QR codes than Zstd-22!
...
cd generator/
python -m src.main generate test_file.txt --compression brotli --fps 2.0
