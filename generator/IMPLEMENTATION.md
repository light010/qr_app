...
│   │       ├── __init__.py
│   │       ├── base_compression.py
│   │       └── brotli_compression.py  # ONLY compression algorithm (Brotli-11)
│   │
...
#### 2.2.1 Compression Service (SIMPLIFIED - BROTLI ONLY)
```python
...
WHY ONLY BROTLI-11?
- BEST compression ratio = minimum number of QR codes (5-10% better than Zstd!)
- Native browser support = scanner needs NO external libraries (simpler air-gap)
- Single code path = easier to verify generator/scanner compatibility
- Fewer dependencies = reduced failure points
- Standard library = no external compression library needed

REMOVED: Zstd, LZ4 (inferior to Brotli compression ratio OR require scanner libraries)
"""
...
class BrotliCompression:
    """
    Brotli compression - ONLY compression algorithm supported

    Level 11 (maximum) provides:
    - 20-30% compression ratio (BEST available - superior to Zstd!)
    - Minimum number of QR codes
    - Native browser decompression (no external libraries needed)
    - Perfect for air-gap deployment (scanner has zero dependencies)
    """

    def __init__(self, quality: int = 11):
        """Initialize with quality 11 (maximum compression) - ALWAYS"""
        self.quality = 11  # Fixed at maximum compression
...
class CompressionService:
    ...
    def __init__(self):
        # SIMPLIFIED: Only Brotli-11 supported
        self.algorithm = BrotliCompression()

    def compress(self, data: bytes) -> bytes:
        """Compress data using Brotli quality 11 (ONLY algorithm)"""
    ...
    @property
    def name(self) -> str:
        """Get algorithm name (always 'brotli')"""
