...
## ⚠️ CRITICAL: Generator Compatibility Requirement

**THIS SCANNER MUST ALWAYS BE ABLE TO SCAN AND RECONSTRUCT QR CODES GENERATED BY THE GENERATOR.**

This is a **military-grade air-gapped system** with **NO fallback mechanism**. If the scanner cannot decode generator output, the entire system fails. There is no network, no server, no patches - generator and scanner MUST remain 100% compatible at all times.
...
**MANDATORY TESTING:**
```bash
# 1. Generate test QR codes with generator
cd ../generator/
python qr_generator.py test-file.pdf

# 2. Scan with scanner
# Open scanner in browser, scan all QR codes

# 3. Verify file reconstruction
# Downloaded file must be byte-for-byte identical to original

# 4. Test with different file types
# Repeat with: .txt, .jpg, .zip, .mp4, .docx
```

**If reconstruction fails → DO NOT DEPLOY. Fix scanner immediately.**

### What Breaks Compatibility

❌ Changing Protocol V3 binary format structure
❌ Removing/changing compression algorithm support
❌ Modifying chunk assembly logic
❌ Breaking binary parser (byte offset errors)
❌ Removing metadata field parsing

### Maintenance Guidelines

**Before ANY scanner changes:**
1. Review generator Protocol V3 specification (ENTERPRISE_GUIDE.md)
2. Run full compatibility test suite (all file types)
3. Verify binary parser matches generator encoder
4. Test on iOS, Android, Desktop browsers
5. Document any protocol changes in BOTH generator and scanner

**Remember: In air-gapped deployment, there is NO second chance.**
